...
export function TODO(props) {
  ...
  useEffect(() => {
    const fetchTodo = async () => {
      const apiData = await getTodo(); // Added await here
      setTodoData(apiData);
      setLoading(false);
    };
    fetchTodo();
  ...
  const updateTodo = id => {
    ...
    const options = {
      ...
      data: {
        done: !todoToUpdate.done, // Ensure we update the 'done' status
      },
    };
    axios
      .request(options)
  ...
  const saveEditTodo = id => {
    const options = {
      method: 'PATCH',
      url: `http://localhost:8000/api/todo/${id}`, // Fixed typo in URL
      headers: {
        accept: 'application/json',
      },
      data: {
